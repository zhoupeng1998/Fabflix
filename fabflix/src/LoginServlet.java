...
public class LoginServlet extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException{
	  // boolean value to check if the user exists
	  exist = true;
      ...
      JSONObject responseObject = new JSONObject();
	   try {
         // if reCaptcha test fail, output the reason.
		   if (userAgent != null && !userAgent.contains("Android") && !RecaptchaUtils.verify(recaptchaResponse)) {
         ...
         if (!checkExistence(username, password, request, responseObject)) {
			   responseObject.put("status","fail");
			   if( !exist )
   ...
   private boolean checkExistence( String user, String pass, HttpServletRequest request, JSONObject responseObject ) throws Exception {
	   // flag value used for a valid user
      ...
      if (userResult.size() > 0) {
	   ...
	   System.out.println("Customer verify " + user + " - " + pass + " " + success);
	   // if a valid user detected
      if (success) {
		   request.getSession().setAttribute("user", new User(user, userResult.getJSONObject(0).get("EMAIL").toString(), userResult.getJSONObject(0).get("ID").toString()));
